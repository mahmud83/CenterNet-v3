...
class MultiPoseLoss(torch.nn.Module):
    def __init__(self, opt):
        ...
        # self.crit_hm_hp = torch.nn.MSELoss() if opt.mse_loss else FocalLoss(None)
...
class MultiPoseDotaFourTrainer(BaseTrainer):
    def __init__(self, opt, model, optimizer=None):
        super(MultiPoseDotaFourTrainer, self).__init__(opt, model, optimizer=optimizer)

    def _get_losses(self, opt):
        loss_states = ['loss', 'hm_loss', 'hp_loss', 'hm_hp_loss',
                       'hp_offset_loss', 'off_loss']
