...
def ctdet_four_decode(heat, kps, reg=None, hm_hp=None, hp_offset=None, K=100):
    batch, cat, height, width = heat.size()
    num_joints = kps.shape[1] // 2
    heat = _nms(heat)
    scores, inds, clses, ys, xs = _topk(heat, K=K)
    ...
    if hm_hp is not None:
        ...
        kps = kps.view(batch, K, num_joints, 2).permute(
            0, 2, 1, 3).contiguous()  # b x J x K x 2
        reg_kps = kps.unsqueeze(3).expand(batch, num_joints, K, K, 2)
        hm_score, hm_inds, hm_ys, hm_xs = _topk_channel(hm_hp, K=K)  # b x J x K   1*4*100
        if hp_offset is not None:
            hp_offset = _tranpose_and_gather_feat(
                hp_offset, hm_inds.view(batch, -1))
            hp_offset = hp_offset.view(batch, num_joints, K, 2)
        ...
        hm_kps = hm_kps.gather(3, min_ind)
        hm_kps = hm_kps.view(batch, num_joints, K, 2)
        ## cal min box and get l t r b
        l = torch.min(kps[:,:,:,0], 1)[0].view(batch, 1, K, 1).expand(batch, num_joints, K, 1)
        t = torch.min(kps[:,:,:,1], 1)[0].view(batch, 1, K, 1).expand(batch, num_joints, K, 1)
        r = torch.max(kps[:,:,:,0], 1)[0].view(batch, 1, K, 1).expand(batch, num_joints, K, 1)
        b = torch.max(kps[:,:,:,1], 1)[0].view(batch, 1, K, 1).expand(batch, num_joints, K, 1)
        mask = (hm_kps[..., 0:1] < l) + (hm_kps[..., 0:1] > r) + \
               (hm_kps[..., 1:2] < t) + (hm_kps[..., 1:2] > b) + \
               (hm_score < thresh) + (min_dist > (torch.max(b - t, r - l) * 0.3))
        mask = (mask > 0).float().expand(batch, num_joints, K, 2)
        # mask1 = hm_score > thresh
        # kps = (1 - mask) * hm_kps + mask * kps
        kps = hm_kps
        # kps = kps.permute(0, 2, 1, 3).contiguous().view(
        #     batch, K, num_joints * 2)
        kps = kps.permute(0, 2, 1, 3).contiguous().view(
            batch, K, num_joints * 2)
    detections = torch.cat([xs, ys, kps, scores, clses], dim=2)
