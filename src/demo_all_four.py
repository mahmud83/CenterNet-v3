...
import shapely
import copy
from show_utils import show_bbox, show_bbox_four, area_and_point_nms
        ...
        for j in range(len(w_overlaps)):
            ...
            for cls in range(1, num_classes + 1):
                for idx, bbox in enumerate(results[key_name[i * w_len + j]][0][cls]):  # 0 represent 1 image
                    if bbox[10] > vis_thresh:
                        bbox[np.arange(0, 10, 2)] = bbox[np.arange(0, 10, 2)] + w_overlaps[j] * 512
                        bbox[np.arange(1, 10, 2)] = bbox[np.arange(1, 10, 2)] + h_overlaps[i] * 512
                        bbox_list = bbox_list + [np.append(np.append(bbox[0:10], bbox[10]), int(cls))]
...
def demo(opt):
    ...
    for (image_name) in image_names:
        ...
        show_bbox_four(bbox_image_total, num_classes, image_show, name_save)
